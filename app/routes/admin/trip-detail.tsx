...
export const loader = async ({ params }: LoaderFunctionArgs) => {
    ...
    // #_! Solusi meningkatkan kecepatan web jika memiliki bnanyak fetch * PENTING
    const [trip, trips] = await Promise.all([
         getTripById(tripId),
         getAllTrips(4, 0)
    ])

    return { trip, allTrips: trips.allTrips.map(({ $id, tripDetails, imageUrls }) => ({
            id: $id,
            ...
            <section className="container wrapper-md">
                ...
                <section className="gallery">
                    {imageUrls.map((url: string, i: number) => (
                        <img src={url} key={i} alt={name} className={cn("w-full rounded-xl object-cover", i === 0 ? "md:col-span-2 md:row-span-2 h-[330px]" : "md:row-span-1 h-[150px]")} />
                    ))}
                </section>
